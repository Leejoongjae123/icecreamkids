...
function GridCElement({
  ...
  // 기본 플레이스홀더 이미지를 사용하지 않음 (초기엔 완전히 비어있도록 처리)
  ...
  // 이미지가 있는지 확인하는 헬퍼 함수 (noimage2.svg 경로는 이미지로 취급하지 않음)
  const isPlaceholder = React.useMemo(() =>
    typeof currentImageUrl === 'string' && currentImageUrl.toLowerCase().includes('noimage2.svg')
  , [currentImageUrl]);
  const hasImage = Boolean(currentImageUrl) && !isPlaceholder;
  ...
  React.useEffect(() => {
    ...
    if (imageUrl && !(typeof imageUrl === 'string' && imageUrl.toLowerCase().includes('noimage2.svg'))) {
    ...
    <div className="relative w-full h-full" style={{ zIndex: toolbarState.show ? 100 : 'auto' }}>
      ...
      >
        ...
        >
          ...
          >
            {hasImage && (() => {
              // 편집 모드와 뷰 모드에서 동일한 VIEW를 유지하기 위해 항상 object-contain 사용
              const imageObjectFitClass = "object-contain";
              return (
                <img
                  src={currentImageUrl}
                  alt="GridC image"
      ...
      {inlineEditState.active && typeof window !== 'undefined' && (() => {
        ...
        return ReactDOM.createPortal(
          <>
            {/* 전체 화면 오버레이: 화면 전체를 덮고, 그리드 영역의 클립 경로만 투명하게 도려냄 */}
            <div className="fixed left-0 top-0 z-[9998] pointer-events-none" style={{ width: "100vw", height: "100vh" }}>
              <svg width={vw} height={vh} viewBox={`0 0 ${vw} ${vh}`}>
                <defs>
                  <mask id={`edit-mask-${gridId}`} maskUnits="userSpaceOnUse">
                    {/* 마스크 기본값: 흰색(오버레이 유지) */}
                    <rect x={0} y={0} width={vw} height={vh} fill="white" />
                    {/* 그리드 위치에 맞게 경로를 배치하고 검정으로 칠해 구멍 생성 */}
                    <g transform={`translate(${rect.left}, ${rect.top}) scale(${rect.width}, ${rect.height})`}>
                      <path d={clipPathData.pathData} fill="black" />
                    </g>
                  </mask>
                </defs>
                {/* 반투명 오버레이에 마스크 적용 */}
                <rect x={0} y={0} width={vw} height={vh} fill="black" fillOpacity={0.4} mask={`url(#edit-mask-${gridId})`} />
              </svg>
            </div>
