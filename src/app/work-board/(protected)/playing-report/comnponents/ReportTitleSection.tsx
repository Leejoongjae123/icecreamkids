...
function ReportTitleSectionImpl({ className = "", initialData }: ReportTitleSectionProps, ref: React.Ref<ReportTitleSectionRef>) {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      if (isPlus) {
        setInlineEditState(prev => ({
          ...prev,
          tempPosition: { ...prev.tempPosition, scale: Math.min(3, (prev.tempPosition.scale || 1) * 1.2) }
        }));
      } else if (isMinus) {
        setInlineEditState(prev => ({
          ...prev,
          tempPosition: { ...prev.tempPosition, scale: Math.max(0.1, (prev.tempPosition.scale || 1) / 1.2) }
        }));
      }
    };
  ...
  // 인라인 편집 활성 시 Enter/ESC 키로 적용/취소 (입력 포커스 시 무시)
  useEffect(() => {
    if (!inlineEditState.active) return;
    const onKeyDown = (e: KeyboardEvent) => {
      const target = e.target as HTMLElement | null;
  ...
  }, [inlineEditState.active, endInlineEditConfirm, endInlineEditCancel, isSaved]);
  ...
  return (
    ...
    >
      ...
      {/* 인라인 편집 확인/취소 버튼 포털 - 읽기전용이 아닐 때만 표시 */}
      {inlineEditState.active && !isSaved && typeof window !== 'undefined' && ReactDOM.createPortal(
