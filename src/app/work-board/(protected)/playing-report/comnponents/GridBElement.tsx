          ...
          {currentImages.map((imageSrc, index) => {
            ...
            return (
              <div key={index} className="w-full h-full">
                <div
                  className={`relative cursor-pointer hover:opacity-80 transition-opacity group w-full h-[110px] rounded-md ${isSaved ? "border-none" : "border border-dashed border-[#AAACB4]"} ${isSaved && !(imageSrc && imageSrc !== "" && imageSrc !== "/report/noimage2.svg") ? "invisible" : ""}`}
                  style={gridAreaStyle}
                  onClick={(e) => {
                    // 클릭 시에도 크기 측정
